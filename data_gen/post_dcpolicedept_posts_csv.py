...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Read DC police posts from CSV and POST them to an inject endpoint."
    )
    parser.add_argument(
        "--csv-file",
        default="dcpolicedept_last_20_posts.csv",
        help="Input CSV path. Default: dcpolicedept_last_20_posts.csv",
    )
    parser.add_argument(
        "--endpoint",
...
def upload_rows(
    ...
    for index, row in enumerate(rows, start=1):
        try:
            ...
            print(f"[{index}] POST {status_code} {row.get('posted_at', '')} {row.get('post_id', '')}")
            if response_body:
            ...
            print(
                f"[{index}] HTTP {exc.code} for post_id={row.get('post_id', '')} posted_at={row.get('posted_at', '')}",
                file=sys.stderr,
            )
            if body:
            ...
            print(
                f"[{index}] Network error for post_id={row.get('post_id', '')} posted_at={row.get('posted_at', '')}: {exc}",
                file=sys.stderr,
            )
            failure_count += 1
            if stop_on_error:
