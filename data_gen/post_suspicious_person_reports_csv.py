...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Read suspicious person reports from CSV and POST them to an inject endpoint."
    )
    parser.add_argument(
        "--csv-file",
        default="suspicious_person_reports_24h.csv",
        help="Input CSV path. Default: suspicious_person_reports_24h.csv",
    )
    parser.add_argument(
        "--endpoint",
        default="http://localhost:8000/api/demo/inject",
        help="Destination endpoint. Default: http://localhost:8000/api/demo/inject",
    )
    parser.add_argument(
        "--timeout-seconds",
        type=float,
...
def resolve_csv_path(csv_file: str) -> Path:
    ...
    if csv_file == "suspicious_person_reports_24h.csv":
        fallback = SCRIPT_DIR / "suspicious_person_reports.csv"
        if fallback.exists():
            return fallback
...
def upload_rows(
    ...
    for index, row in enumerate(rows, start=1):
        try:
            ...
            print(f"[{index}] POST {status_code} {row.get('occurred_at', '')} {row.get('report_id', '')}")
            if response_body:
            ...
            print(
                f"[{index}] HTTP {exc.code} for report_id={row.get('report_id', '')} occurred_at={row.get('occurred_at', '')}",
                file=sys.stderr,
            )
            if body:
            ...
            print(
                f"[{index}] Network error for report_id={row.get('report_id', '')} occurred_at={row.get('occurred_at', '')}: {exc}",
                file=sys.stderr,
            )
            failure_count += 1
            if stop_on_error:
