...
- OSINT stub or contextual evidence drawer
- any small static map used only for internal debugging

### What must not appear in the first demo screen
- empty case queue
- freeform chat box
...
## 13. Optional OSINT contract

If OSINT is enabled in a deployment, it must follow all of these rules:
- configurable by deployment
- provenance-preserving
- collapsed by default in the UI
- not required in the gold demo
- not allowed to open a case by itself
- not allowed to move the case to **Verify Now** by itself
- not allowed to move the case to **Escalate Now** by itself
- not allowed to replace the suspicious-person report in the MVP
- not allowed to be more prominent than the three core evidence types

## 14. Build-readiness verdict

**Ready for MVP Build + Demo handoff.**
