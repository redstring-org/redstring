...
### Forbidden UI and behavior
- The product works without queue-first UX.
- The product works without map-first UX.
- The product works without chat UX.
- The product does not require a visible numeric score.
- The product does not require OSINT to explain the case.

### Failure handling
- If AI phrasing fails, deterministic template text still renders the case correctly.
- The case remains usable if optional OSINT is unavailable.
...
## Optional OSINT acceptance criteria
- OSINT can be disabled without affecting any core MVP behavior.
- OSINT is configurable by deployment.
- OSINT preserves provenance: source, time, URL, and location hint.
- OSINT is collapsed by default in the UI.
- OSINT is not required in the gold demo.
- OSINT does not open a case by itself.
- OSINT does not change the state by itself.
- OSINT does not justify escalation by itself.
- OSINT is never more prominent than the three core evidence types.
